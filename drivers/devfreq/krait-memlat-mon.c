...
/* Taken from perf_event_v7.c */
#define ARMV7_PERFCTR_INSTR_EXECUTED		0x08
#define ARMV7_PERFCTR_MEM_ACCESS			0x13
#define ARMV7_PERFCTR_CPU_CYCLES			0xFF

/* Values mirroring arm-memlat-mon.c */
#define INST_EV		PERF_COUNT_HW_INSTRUCTIONS
#define L2DM_EV		ARMV7_PERFCTR_MEM_ACCESS
#define CYC_EV		PERF_COUNT_HW_BUS_CYCLES
...
struct cpu_grp_info {
	...
	struct notifier_block krait_memlat_cpu_notif;
};
...
static void stop_hwmon(struct memlat_hwmon *hw)
{
	...
	unregister_cpu_notifier(&cpu_grp->krait_memlat_cpu_notif);
}
...
static int krait_memlat_cpu_callback(struct notifier_block *nb,
...
static int start_hwmon(struct memlat_hwmon *hw)
{
	...
	register_cpu_notifier(&cpu_grp->krait_memlat_cpu_notif);
...
static int krait_memlat_mon_driver_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct memlat_hwmon *hw;
	...
	if (!cpu_grp)
		return -ENOMEM;
	cpu_grp->krait_memlat_cpu_notif.notifier_call = krait_memlat_cpu_callback;
	hw = &cpu_grp->hw;
...
static struct platform_driver krait_memlat_mon_driver = {
	.probe = krait_memlat_mon_driver_probe,
	.driver = {
		.name = "arm-memlat-mon",
...
static int __init krait_memlat_mon_init(void)
{
	return platform_driver_register(&krait_memlat_mon_driver);
}
module_init(krait_memlat_mon_init);

static void __exit krait_memlat_mon_exit(void)
{
	platform_driver_unregister(&krait_memlat_mon_driver);
}
module_exit(krait_memlat_mon_exit);
#endif
